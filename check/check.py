...
class Check():
    ...
    def get_network_check(self):
        ...
        self.unit_now = unit           
        try:
            self.baidu = requests.get("http://www.baidu.com", timeout=10).status_code
        except:
            self.baidu = 404
            salmon.logger.warning("Baidu request failed.")
        try:
            self.google = requests.get("http://www.google.com", timeout=10).status_code
        except:
            self.google = 404
            salmon.logger.warning("Google request failed.")
