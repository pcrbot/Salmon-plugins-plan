...
update = on_command('更新卡池')
update_force = on_command('强制更新卡池')
...
async def get_online_pcrdata():
    ...
    '''
    online_pcrdata = await aiohttpx.get(url=online_pcr_data_url, timeout=10)
    if online_pcrdata.status_code != 200:
        salmon.logger.error(f'获取在线角色数据时发生错误{online_pcrdata.status_code}')
        return {}
    # 移除开头的'CHARA_NAME = ', 格式化为json便于处理
    online_pcrdata_text = online_pcrdata.text
...
def ids2names(ids: list) -> list:
    ...
    for id in ids:
        if id in _pcr_data.CHARA_NAME:
            ...
            salmon.logger.warning(f'缺少角色{id}的信息, 请注意更新静态资源')
    return res
...
def get_local_ver() -> int:
    ...
    if os.path.exists(local_ver_path):
        ...
        local_ver = 0
    salmon.logger.info(f'检查卡池更新, 本地卡池版本{local_ver}')
    return local_ver
...
async def get_online_ver() -> int:
    ...
    获取在线版本号
    '''
    online_pool_ver = await aiohttpx.get(url=online_ver_url, timeout=10)
    if online_pool_ver.status_code != 200:
        salmon.logger.error(f'获取在线卡池版本时发生错误{online_pool_ver.status_code}')
        return online_pool_ver.status_code
    online_pool_ver_json = online_pool_ver.json()
    online_ver = int(online_pool_ver_json["ver"])
    salmon.logger.info(f'检查卡池更新, 在线卡池版本{online_ver}')
    return online_ver
...
def update_local_ver(ver: int) -> None:
    ...
    salmon.logger.info(f'写入本地版本号文件')
...
async def get_online_pool():
    ...
    获取在线卡池, 返回json格式
    '''
    salmon.logger.info(f'开始获取在线卡池')
    online_pool_f = await aiohttpx.get(online_pool_url, timeout=10)
    if online_pool_f.status_code != 200:
        salmon.logger.error(f'获取在线卡池时发生错误{online_pool_f.status_code}')
...
@update.handle()
async def update_pool_chat(bot: Bot, event: CQEvent):
    '''
    手动更新卡池时试用此命令
    '''
    ...
    if status == 0:
        await update.finish('已是最新版本, 仍要更新卡池请使用【强制更新卡池】命令')
    elif status < 1000:
        await update.finish(f'发生错误{status}')
    else:
        await update.finish(f'更新完成, 当前卡池版本{status}')


@update_force.handle()
async def update_pool_force_chat(bot: Bot, event: CQEvent):
    '''
    强制更新卡池
    '''
    if not priv.check_priv(event, priv.SUPERUSER):
    ...
    if status == 0:
        await update_force.finish(f'状态{status}')
    elif status < 1000:
        await update_force.finish(f'发生错误{status}')
    else:
        await update_force.finish(f'更新完成, 当前卡池版本{status}')


@scheduler.scheduled_job('cron', id='卡池更新', hour='17', minute='05')
async def update_pool_sdj():
    for bot in salmon.get_bot_list():
        master_id = salmon.configs.SUPERUSERS[0]
        status = await update_pool()
        if status == 0:
            return
