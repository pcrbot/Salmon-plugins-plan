...
async def get_pic(address):
    ret = await aiohttpx.get(address, timeout=20)
    return ret.content
...
class ascii2d():
    ...
    async def get_search_data(self, url: str, data=None):
        if data is not None:
            ...
            html_data = await aiohttpx.get(url, timeout=15)
    ...
    async def get_view(self, ascii2d) -> str:
        ...
        try:
            html_index_data = await aiohttpx.get(url_index, timeout=7)
...
async def get_image_data_sauce(image_url: str, api_key: str):
    ...
    salmon.logger.info("Loading Image Search Container……")
    NAO = SauceNAO(api_key, numres=SAUCENAO_RESULT_NUM)
    salmon.logger.debug("Loading all view……")
    repass = ''
    simimax = 0
    try:
